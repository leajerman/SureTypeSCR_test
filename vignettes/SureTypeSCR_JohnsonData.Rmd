...
Now we perform MA transformation. MA transformation displays intensities as logarithmic average (A) and logarithmic difference (M). Roughly speaking, M\~0 corresponds to heterozygous genotypes, M\<0 corresponds to homozygous B and M\>0 corresponds to homozygous A.

```{r  MAtransform,echo=TRUE,results="show",include=TRUE}
df %>%
  calculate_ma() %>%
  plot_ma(n=0.1) + geom_jitter(aes(col))
...
```{r Subset_classification,echo=TRUE,results="show",include=TRUE}
...
```{r  Subset_score_distribution,echo=TRUE,results="show",include=TRUE}
df_model %>%
  unnest(c(data ,model)) %>%
  group_by(individual)  %>%
